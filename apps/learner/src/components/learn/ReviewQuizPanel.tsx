...
export default function ReviewQuizPanel({ sectionId, questions, onResult }: ReviewQuizPanelProps) {
  const { submitReviewQuiz, getLastQuizAttempt, hasPassedReviewQuiz } = useWorkspace();
  ...
  if (hasPassedReviewQuiz(sectionId)) {
    return (
      <div className="rounded-2xl border border-zinc-200 bg-white p-5 dark:border-zinc-800 dark:bg-zinc-900">
        <div className="flex items-center gap-2 text-green-700 dark:text-green-400">
          <CheckCircle2 className="h-5 w-5" />
          <h2 className="text-lg font-semibold">Review quiz passed</h2>
        </div>
        <p className="mt-2 text-sm text-zinc-600 dark:text-zinc-400">
          You can now retry the section quiz.
        </p>
      </div>
    );
  }
  ...
  const wrongQuestionIds = new Set(
    questions
      .filter(
        (q) =>
          lastAttempt.answers[q.id]?.toLowerCase() !== q.correctAnswer.toLowerCase()
      )
      .map((q) => q.id)
  );
    ...
    return (
      <div className="rounded-2xl border border-zinc-200 bg-white p-5 dark:border-zinc-800 dark:bg-zinc-900">
        <p className="text-sm text-zinc-600 dark:text-zinc-400">
          No review questions available. You can retry the quiz.
        </p>
      </div>
    );
  }
  ...
  const score = reviewQuestions.filter(
    (q) => answers[q.id]?.toLowerCase() === q.correctAnswer.toLowerCase()
  ).length;
  const passed = reviewQuestions.length === 0 ? true : score / reviewQuestions.length >= 0.7;

  const handleSubmit = () => {
    if (!allAnswered) return;
    submitReviewQuiz(sectionId, lastAttempt.id, answers, reviewQuestions);
    setSubmitted(true);
    onResult?.(passed);
  };

  return (
    <div className="rounded-2xl border border-amber-200 bg-white p-5 dark:border-amber-900/50 dark:bg-zinc-900">
      <div className="mb-5 flex items-center justify-between">
        <div>
          <h2 className="text-xl font-bold text-zinc-900 dark:text-white">Review quiz</h2>
          <p className="text-sm text-zinc-500 dark:text-zinc-400">
            Focused on the {reviewQuestions.length} question{reviewQuestions.length > 1 ? "s" : ""} you missed.
          </p>
        </div>
        {submitted && (
      ...
      {!submitted ? (
        ...
        <div className={`mt-6 rounded-lg p-4 text-center ${
          ...
          <p className={`text-lg font-semibold ${
            ...
            {passed ? "Review complete!" : "Keep reviewing"}
          </p>
          <p className="mt-1 text-sm text-zinc-600 dark:text-zinc-400">
            {passed
              ? "You can now retake the section quiz."
